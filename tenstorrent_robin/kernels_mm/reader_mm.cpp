...
// Reader du matmul single-core officiel tt-metal (inchangé) — lit les tuiles A (M,K)
// et B (K,N) depuis la DRAM et les pousse dans les circular buffers.
#include <stdint.h>
    ...
    for (uint32_t mt = 0; mt < Mt; mt++) {
        for (uint32_t nt = 0; nt < Nt; nt++) {
            for (uint32_t kt = 0; kt < Kt; kt++) {
                {   // A tile (mt, kt) — A est M×K, stride Kt
                ...
                {   // B tile (kt, nt) — B est K×N, stride Nt
