...
This exact filename appears in one summary table of the project spec (an
earlier, generic overview section listing `results.csv` / `resource_report.md` /
`scaling_analysis.md`). The spec's own detailed, week-by-week deliverable
...
The two names appear to be an internal inconsistency within the spec document
...
- **Matrix-size scaling analysis, latency model, memory bottleneck
  identification (the Week 7 spec task):**
...
- **Extended scaling analysis — TerEffic comparison, resource wall,
  ...
  (Week 8/9 extension beyond the Week 7 spec ask):**
