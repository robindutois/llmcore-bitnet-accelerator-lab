// =============================================================================
// testbench.cpp
// HLS Testbench — BitLinear-FPGA Alpha
// =============================================================================
// Covers both C-Simulation (Week 3) and RTL Co-Simulation (Week 5).
//
// Validation chain closed:
//   Python ref (S2) → .bin files → C++ ref → HLS C-sim (S3) → RTL (S5)
//
// Runs 3 checks per test vector:
//   [A] HLS output   vs expected_output_int32.bin  (Python ground truth)
//   [B] C++ ref      vs expected_output_int32.bin
//   [C] HLS output   vs C++ ref
//
// Co-sim compatibility (WRAPC rules):
//   - No VLAs — all buffers are static globals
//   - HLS interface buffers sized to pragma depth values:
...
//
// Standalone compile (C-sim without Vitis):
//   g++ -o testbench testbench.cpp \
...
// =============================================================================
// TV_BASE_PATH — written by run_hls.tcl before csim/cosim
// =============================================================================
...
// Buffer dimensions
//
// HLS interface buffers — must match pragma depth in bitlinear_hls.cpp:
...
#define HLS_DEPTH_W_PACK  131072  // pragma depth for W_packed (uint8_t) — 128 KB
...
// Non-HLS buffers — sized to actual test vector maximums
static int8_t   s_W_unpacked[TV_MAX_MK];         // 131 072 bytes
static int32_t  s_y_ref     [TV_MAX_M];          // 1 024 bytes
static int32_t  s_y_expected[TV_MAX_M];          // 1 024 bytes

// =============================================================================
// C++ golden reference (same logic as bitlinear_reference.cpp — Week 2)
// =============================================================================
...
static int run_test_vector(const char* base_path, const char* test_name) {
    ...
    // Zero HLS-interface buffers before loading (WRAPC reads full depth)
...
// =============================================================================
int main() {
    printf("=== HLS BitLinear Testbench — Week 2 Test Vector Validation ===\n");
    ...
    if (!check) {
        ...
        printf("  Set TV_BASE_PATH to the absolute path of reference/test_vectors/\n");
        return 1;
    }
    fclose(check);
    ...
    if (passed == total) {
        printf("STATUS: PASS\n");
        printf("  Chain verified: Python(S2) -> .bin -> C++ ref -> HLS C-sim(S3) -> RTL(S5)\n");
        return 0;
    }
